...
dx %>% 
  ggplot(aes(x = reorder(str_c("<b>", nom_mun, "</b>", "<br>", str_to_title(ENTIDAD)), TOTAL_OPINIONES),
             y = TOTAL_OPINIONES)) + 
  geom_col(fill = "brown",
           alpha = 0.5) + 
  geom_hline(yintercept = 40, linetype = 2, color = "blue", size = 1, alpha = 0.5) +
  geom_text(aes(label = str_c(prettyNum(round(TOTAL_OPINIONES), big.mark = ","), "")), 
            fontface = "bold", 
            color = "brown",
            family = "Poppins", 
            size = 4, hjust = -0.1, 
            show.legend = FALSE) +
  coord_flip() + 
  labs(x = "", y = "Opiniones", title = "Top 20 municipios con mayor numero de opiniones emitidas<br>en la Consulta Popular 2021", 
